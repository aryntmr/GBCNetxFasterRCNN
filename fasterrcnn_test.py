...
import fasterrcnn_transforms as T
import utils
from torch.optim.lr_scheduler import StepLR
from skimage import io, transform
import sys
sys.path.insert(0, r"C:\Users\arynt\OneDrive\Desktop\IIT Delhi\Faster-RCNN\Faster-RCNN")
from fasterrcnn_dataloader import GbRoiDatasetTest, GbClassDataset, GbCustomLoader
...
def parse():
    ...
    parser.add_argument('--meta_file', dest="meta_file", default="data/bbox_annot.json")
    parser.add_argument('--label_file', dest="label_file", default="data/test.txt")
    ...
    parser.add_argument('--save_dir', dest="save_dir", default="gb_data/out")
    parser.add_argument('--save_name', dest="save_name", default="mod")
    parser.add_argument('--model_dir', dest="model_dir", default=r"C:\Users\arynt\OneDrive\Desktop\IIT Delhi\Faster-RCNN\Faster-RCNN\weights")
...
def main(args):
    ...
    with open(val_set_file, "r") as f:
        for e in f.readlines():
            ...
            fnames.append(e)
    # dset = {}
    # list_imgs = os.listdir(args.img_dir)
    ...
    weight_file = "%s/fasterrcnn-roi.pth"%(args.model_dir)#, args.set_num)
    ...
    for images, targets, image_ids in loader:
        images = [image.float().cpu() for image in images]
        ...
        for i, image in enumerate(images):
            ...
            }                                                                   # Removed Gold key from the dictionary
    ...
    fname = os.path.join(args.out_dir, "roi_pred%s.json")
    
    save_stat(d, fname, args)



# Temporary function to return the results of fasterRCNN to the GBCNet
def temp(img_dir):
    args = parse()
    #os.system('')
    if args.resize:
    ...
    dataset = GbCustomLoader(img_dir, dset, img_transforms=img_transforms)
    loader = DataLoader(dataset, batch_size=1, shuffle=False, collate_fn=utils.collate_fn)
...
if __name__ == "__main__":
    ...
    # #print(args)
    # main(args)
    temp(r"C:\Users\arynt\OneDrive\Desktop\IIT Delhi\sample_data\images")
