...
from dataloader import GbDataset, GbRawDataset, GbCropDataset, GbCustomLoader
...
import sys
source_dir = r"C:\Users\arynt\OneDrive\Desktop\IIT Delhi\Faster-RCNN\Faster-RCNN"
sys.path.append(source_dir)
from fasterrcnn_test import temp
...
def parse():
    ...
    parser.add_argument('--load_path', dest="load_path", default=r"C:\Users\arynt\OneDrive\Desktop\IIT Delhi\gbcnet.pth")
...
def main(args):
    ...
    # Additional df processing
    # df_temp = {}
    # for img in df:
    #     bb_idx = len(img['Boxes']) - 1
    #     d_temp = img
    #     d_temp['bb_idx'] = bb_idx
    #     d_temp['Labels'] = int(d_temp['Labels'])
    #     df_temp[img['image_id']] = d_temp
    # df = df_temp
    #print("df_new",df)
    ...
    if args.no_roi:
        ...
        #val_dataset = GbCustomLoader(args.img_dir, df, to_blur=args.to_blur, sigma=args.sigma, p=args.patch, img_transforms=val_transforms)
    ...
    net.load_state_dict(torch.load(args.load_path, map_location=torch.device('cpu')))

    net.net = net.net.float().cpu()
    ...
    for images, targets, filenames in val_loader:
        images, targets = images.float().cpu(), targets.cpu()
    ...
    # Modified training loop without target
    # image_ids = []
